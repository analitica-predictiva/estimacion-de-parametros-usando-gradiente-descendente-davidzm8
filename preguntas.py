...
def pregunta_02():
    ...
    for epoch in range(n_iterations):
        ...
        grad_w2 = -2*sum([error*x_valor[0] for error, x_valor in zip(error,x_poly)])
        grad_w1 = -2*sum([error*x_valor[1] for error, x_valor in zip(error,x_poly)])
        grad_w0 = -2*sum([error*x_valor[2] for error, x_valor in zip(error,x_poly)])
        
        gradient = np.array([grad_w2,grad_w1,grad_w0])
